...
fn splice_optim(input: &Input, actions: &mut [Action], max_size: usize)
{
	...
	loop
	{
			...
			for slice_end in slice_start+2..actions.len().min(slice_start+2+max_size)
			{
				...
				for d in 1..delta
				{
					...
					if new_value > value
					{
						value = new_value;
						best_d = d;
						println!("Splice optim found {new_value:?}");
						res = true;
					}
				}
		...
		for center in 1..actions.len()-1
		{
			let max_d = usize::min(max_size, usize::min(center, actions.len() - center - 1));
			let mut best_d = 0;
			for d in 1..=max_d
			{
				actions.swap(center - d, center + d);

				if !check_actions_at(input, actions, center - d) || !check_actions_at(input, actions, center + d)
				{
					continue;
				}
				...
				if new_value > value
				{
					value = new_value;
					best_d = d;
					println!("Swap optim a found {new_value:?}");
					res = true;
				}
			}

			for d in (best_d+1)..=max_d
			{
				actions.swap(center - d, center + d);
			}
		}
		...
		for center in 1..actions.len()
		{
			let max_d = usize::min(center, actions.len() - center);
			let mut best_d = 0;
			for d in 1..=max_d
			{
				actions.swap(center - d, center + d - 1);

				if !check_actions_at(input, actions, center - d) || !check_actions_at(input, actions, center + d - 1)
				{
					continue;
				}
				...
				if new_value > value
				{
					value = new_value;
					best_d = d;
					println!("Swap optim b found {new_value:?}");
					res = true;
				}
			}
